...
public class GradesAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if(getItemViewType(cursor.getPosition())!=VIEW_TYPE_FIRST) {
            if (isDifferentWithNext(casperString, cursor)) {
                view.setBackgroundResource(R.drawable.bottom_line_view); // Ajoute le séparateur noire
            } else {
    ...
    private boolean isDifferentWithNext(String casperString, Cursor cursor) {
        if(cursor != null && cursor.moveToNext() && !casperString.equals(cursor.getString(GradesFragment.COL_GRADE_CASPER))){
